...
SLW models are similar to WSGG models but instead of integrating over simple sequential $\eta$ bands, spectral integration is done over wavenumbers in a given range of absorption cross section. So, in Eq.~(\ref{e:RTE_wsgg}), gas $j$ would refer to wavenumbers $\eta\in\Delta_j$ with $\Delta_j=\{\eta:\,\tilde{C}_{j-1}<C_\eta<\tilde{C}_j\}$, using Solovjov's notation, where $C$ denotes absorption cross-section (where $\kappa=CN$, $N$ is molar density of the radiatively participating gas), and $\tilde{C}$ are supplemental cross sections defining interfaces between adjacent cross-section bands. 

If $\tilde{C}_j$ are known for all $j$, then $\kappa_j = N\sqrt{C_{j-1}C_j}$. The weights $a_j$ are computed using the absorption line blackbody distribution function (ALBDF), denoted $F$, which gives the fraction of total blackbody emission from $\eta$ corresponding to absorbtion cross sections less than $C$: $F$ is a monotonically-increasing function of $C$, and depends on the thermodynamic state $\phi$, and the black temperature $T_b$ defining the Planck function: $F = F(C,\phi,T_b)$. The $a_j$ are then computed as
%
\begin{linenomath}
\begin{equation}
...
Equation~(\ref{e:RTE_wsgg}) assumes the $\Delta_j$ intervals are independent of spatial position, but care must be taken since $C(\eta)$ depends on the thermodynamic state, which does vary spatially. Hence, the fixed $\Delta_j$ imply a corresponding variation in $C_j$ and $\tilde{C}_j$. 
