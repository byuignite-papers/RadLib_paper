...
\begin{linenomath}
    \begin{equation} \label{e:albdf}
        F(C,\BS{\phi},T) = \frac{1}{\sigma T^4}\int_{\{\eta:\,C(\eta,\BS{\phi})<C\}}E_b(\eta,T)d\eta,
\end{equation}
\end{linenomath}
%
so that
...
Equation~(\ref{e:RTE_wsgg}) assumes the sets $\Delta_j$ are independent of spatial position $\mathbf{x}$, but care must be taken since C depends on the thermodynamic state (denoted by vector $\BS{\phi}$ with nominal components $T$, $P$, and $y_i$), which does vary spatially. That is, $C=C(\eta,\BS{\phi}(\mathbf{x}))$. Hence, the fixed $\Delta_j$ imply a corresponding spatial variation in $C_j$ and $\tilde{C}_j$.
 
The RCSLW model uses the notion of \emph{rank correlation} to account for this variation in the $C_j$. Solovjov defines two spectra arising from thermodynamic states $\BS{\phi}_1$ and $\BS{\phi}_2$ to be rank correlated if, for an arbirary wavenumber $\hat{\eta}$, the wave number sets $\Delta_1$ and $\Delta_2$ are equal, where $\Delta_1=\{\eta: C(\eta,\BS{\phi}_1)<C(\hat{\eta},\BS{\phi}_1)\}$, and $\Delta_2=\{\eta: C(\eta,\BS{\phi}_2)<C(\hat{\eta},\BS{\phi}_2)\}$. 
Note in Eq.~(\ref{e:albdf}), that $\BS{\phi}$ determines $C$, which then determines the set of $\eta$ integrated over. But for a \emph{given} set $\eta$, e.g., $\Delta_1$, $F$ can be considered as $F(C,T)$. Hence, if $\Delta_1=\Delta_2$, then at a fixed reference black temperature $T_{ref}$, we have $F(C_1,\BS{\phi}_1,T_{ref})=F(C_2,\BS{\phi}_2,T_{ref})$. One can then compute the inverse to find $C_1$ at $\BS{\phi}_1$ and $C_2$ at $\BS{\phi}_2$ (i.e., values at two spatial positions) corresponding to the same wavenumber interval. 
