...
%$I_{b\eta}$ is the Planck spectral distribution of blackbody intensity, 
...
\begin{equation}
 b_{i,j}=\sum_{k=0}^{4}C_{i,j,k}M_r^k,
\end{equation}
where $C_{i,j,k}$ is another correlated model coefficient and $M_r$ is the molar ratio $Y_{\mathrm{H_2O}}/Y_{\mathrm{CO_2}}$ as above. The model coefficients $d_{i,k}$ and $C_{i,j,k}$ can be found in the literature \citep{Bordbar_2014,Bordbar_2020}. 
...
\begin{equation}
\label{e:soot1}
	\kappa_{\lambda}=C_0 \frac{f_v}{\lambda} , \; \; \; C_0 = \frac{36\pi n k}{(n^2-k^2+2)^2+4n^2k^2},
\end{equation}
where $n$ and $k$ are the real and imaginary parts of the soot complex index of refraction, respectively, and $f_v$ is the soot volume fraction \cite{Solovjov_2001,Modest_2013}. Various values for the refraction index have been proposed and applied to various scenarios \cite{Lee_1981,Stull_1960,Dalzell_1969,Howarth_1966,Chang_1990,Felske_1984,Williams_2007}. RadLib uses the values $n=1.75$ and $k=1.03$ given in \cite{Williams_2007}, which result in a value of $C_0=7.03$, aligning nicely with the value $C_0=7.0$ suggested by \cite{Solovjov_2001}. Due to the simple $1/\lambda$ dependence and the suitably small difference between the mean absorption coefficients calculated for optically thin and optically thick cases, we can use one expression to calculate a spectral mean absorption coefficient for soot that applies to all optical regimes:
\begin{equation}
\label{e:soot2}
	\kappa_{soot}=3.72\frac{f_vC_0T}{C_2},
\end{equation}
where $T$ is the temperature and $C_2=1.4388$ cm K is a Planck function constant \cite{Modest_2013}. 

In order to incorporate its radiative effects, soot is modeled as a pseudo-gas of its own and included alongside the existing gray gases. In the Planck Mean model, which models a gas mixture as one gray gas, the soot absorption coefficient calculated from Equation \ref{e:soot2} is simply added to the existing gas absorption coefficient. In the WSGG model, the soot absorption coefficient is similarly calculated from Equation \ref{e:soot2} but is then added to each of the gases separately. Because soot radiates across all wavelengths, its absorption coefficient is added to the values of all four gray gases and the transparent gas that represents the spectral gaps between absorption bands. Unlike the Planck Mean and WSGG models, the RCSLW model requires a spectrally dependent soot model. Instead of finding a mean soot absorption coefficient, we use Equation \ref{e:soot1} to define a soot absorption cross-section and ALBDF as detailed in \cite{Solovjov_2001}. The soot ALBDF can then be multiplied with the other gray gas ALBDFs to find the overall ALBDF for the gas-soot mixture. 
