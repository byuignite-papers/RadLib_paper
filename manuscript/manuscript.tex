...
The correlations provided by Bordbar et~al. (2014) \cite{Bordbar_2014} are valid for $0.01\le M_r\le4$. In Bordbar et~al. (2020) \cite{Bordbar_2020} this range was extended to arbitrary $M_r$. This was done by interpolation of $\kappa_j$ and $a_j$ between the pure component values and the corresponding limit on $M_r$. That is, for \ce{CO2}, $\kappa_j$ and $a_j$ are interpolated between values at $M_r=0.01$ and pure \ce{CO2}. For \ce{H2O}, $\kappa_j$ and $a_j$ are interpolated between values at $M_r=4$ and pure \ce{H2O}.
...
%%%%% DOL original, with VBL suggested edits
%  SLW models are similar to WSGG models, but instead of integrating within a sequential band of $\eta$ to define gray gas $j$, spectral integration is done over a set of (nonsquential) wavenumbers constituting a given band $j$ of absorption cross section $C_\eta$. 
%  %
%  (The absorption coefficient is related to absorption cross section by $\kappa_\eta=C_\eta N$, where $N$ is the molar density of the radiatively participating gas.)
%  %
...
%  %
%  The SLW gases, then, are gray in a given band by construction, to within the width $\tilde{C}_j-\tilde{C}_{j-1}$. In contrast, in traditional WSGG approaches, the gases are only conceptually gray, and the absorption coefficients and weights are determined empirically \cite{Badger_2019}. 



%%%%% DOL restructuring paragraph based on VBL suggestions
...
The RCSLW model uses the notion of rank correlation to compute this variation in $C_j$ and $\tilde{C}_j$. Solovjov et~al.~\cite{Solovjov_2017} define two spectra at states $\BS{\phi}_1$ and $\BS{\phi}_2$ to be rank correlated 
if the set $H_1=\{\eta: C_\eta(\eta,\BS{\phi}_1)<C_\eta(\hat{\eta},\BS{\phi}_1)\}$ is equal to the set
$H_2=\{\eta: C_\eta(\eta,\BS{\phi}_2)<C_\eta(\hat{\eta},\BS{\phi}_2)\}$, for an arbitrary $\hat{\eta}$.
 
Consider Eq.~\ref{e:albdf} at fixed argument $T=T_r$, and let $\hat{C}_1\equiv C_\eta(\hat{\eta},\BS{\phi}_1)$, and $\hat{C}_2\equiv C_\eta(\hat{\eta},\BS{\phi}_2)$; then we have
%
\begin{linenomath}
...
In the RCSLW model for a given problem, a collection of $\tilde{F}_j$ values are specified, with $F_j$ points between $\tilde{F}_j$ and $\tilde{F}_{j-1}$; these apply to all spatial positions. The corresponding $\tilde{C}_j$ and $C_j$ values at any spatial position (and corresponding thermodynamic state $\BS{\phi}$) can then be found by inversion: $C_j = C(F_j,\BS{\phi},T_r)$, and similarly for $\tilde{C}_j$ in terms of $\tilde{F}_j$. $\kappa_j$ is then computed as $\kappa_j=C_jN$, and $a_j$ is computed from Eq.~\ref{e:aj}, where $T$ is taken as the local temperature, not $T_r$. The system average temperature may be used for $T_r$, as done in \cite{Solovjov_2001}, and in the examples below.
