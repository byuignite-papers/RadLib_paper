...
\begin{figure}
    ...
    \begin{tabular}{c c}
        ...
        \includegraphics[width=2.75 in]{ex_1.pdf} &
        \includegraphics[width=2.75 in]{ex_2b.pdf} \\
        Example S3 & Example S4 \\
        \includegraphics[width=2.75 in]{ex_3a.pdf} &
        \includegraphics[width=2.75 in]{ex_4a.pdf} \\
...
\begin{figure}
    ...
    \end{tabular}
    \caption{Computational cost to evaluate gas properties (a) and to run Example S3. Results are normalized by the cost of the WSSG model.}
    \label{f:cost}
    \end{center}
\end{figure}
%
...
Cost comparisons were also attempted with Python, but it is difficult to get meaningful results when mixing Python and C++. In particular, the cost of a single evaluation of the gas properties is small end valuating the gas properties in a Python loop results in the loop appearing to dominate the computational cost. Comparison between Python and C++ for Example S3 is somewhat more meaningful, but even here, for the six cases considered in Fig.~\ref{f:cost} the cost only varied by about 4\%. The cost of the WSGG model was 0.6~s, which is 7.6 times slower than the C++ version. It is possible that improvements in the Python speed are possible by further optimization of the Cython interface, but the Python radiative solver used in the examples is only meant to demonstrate the use of the library evaluation of gas properties.
