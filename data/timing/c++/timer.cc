...
int main() {
    ...
    rad *rc2        = new rad_rcslw(2, P, T, xH2O, xCO2, xCO, fvsoot);
    ...
    double d1, d2;
    double time;
    vector<double> pm_times, ws_times, rc_times, rc8_times, rc2_times;
    double ave = 0;
    ...
    c1 = clock();
    for(int i=0; i<n_sim; i++) {
	    rc2->get_k_a(T, P, xH2O, xCO2, xCO, xCH4, fvsoot, kabs_pm,    awts_pm);
	    //dumb += kabs_pm[1];
	    // cout << " " << setw(6) << kabs_pm[0];
    }
    c2 = clock();
    ...
    time = (d2-d1)/CLOCKS_PER_SEC;
    ave = time/n_sim;
    cout << endl << "RCSLW 2gas average time = " << ave ;
