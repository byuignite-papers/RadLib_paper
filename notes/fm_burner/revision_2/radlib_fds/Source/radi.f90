...
use rad_module                               ! dol
use, intrinsic :: ISO_C_Binding, only: C_ptr !dol
...
type(C_ptr) :: r_radlib !dol
REAL(EB) :: rho_soot    !dol
REAL(EB) :: kabs        !dol
REAL(EB) :: awts        !dol
...
!rho_soot = 1850.0               !dol
if (WSGG_MODEL) THEN            !dol
    if (RCSLW_MODEL) THEN            !dol
        call rad_rcslw(r_radlib, 4, 1.2D3, 1.01325D5, 1.0D-6, 2.0D-1, 1.0D-1, 0.0D0)  !dol
    else                               !dol
        call rad_wsgg(r_radlib)        !dol
    endif                              !dol
endif                                  !dol
...
BAND_LOOP: DO IBND = 1,NUMBER_SPECTRAL_BANDS
   ...
   ELSEIF (WSGG_MODEL) THEN
     ...
     !vvvvvvvvv dol
      ...
      if (IBND==5) then         ! FDS has the clear gas last, radlib clear gas is first: FDS: 1,2,3,4,(5); radlib (0),1,2,3,4 (indicies match except clear)
          call get_k_a_oneband(r_radlib, kabs, awts, 0,    TMPA, 101325.0_EB, 0.0_EB, X_H2O, X_CO2)
      else
          call get_k_a_oneband(r_radlib, kabs, awts, IBND, TMPA, 101325.0_EB, 0.0_EB, X_H2O, X_CO2)
      endif
      BBFA = awts
     !^^^^^^^^^^ dol

     !BBFA = A_WSGG(TMPA,MOL_RAT,IBND)    ! dol comment this
      ...
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
               ...
               !BBF = A_WSGG(TMP(I,J,K),MOL_RAT,IBND)    ! dol comment this                                     ! Temp coefficient for the jth gas
               CALL GET_MASS_FRACTION(Z_ARRAY,SOOT_INDEX,SOOT_MASS_FRACTION)

               !KAPPA_GAS(I,J,K) = KAPPA_WSGG(X_H2O, X_CO2,MOL_RAT,PARTIAL_P,IBND) + &   ! dol comment this
               !    KAPPA_SOOT(RHO(I,J,K)*SOOT_MASS_FRACTION,TMP(I,J,K))          ! dol comment this! Absorp coeff for the jth gas




               if (IBND==5) then         ! FDS has the clear gas last, radlib clear gas is first: FDS: 1,2,3,4,(5); radlib (0),1,2,3,4 (indicies match except clear)
                   call get_k_a_oneband(r_radlib, kabs, awts, 0, TMP(I,J,K), TOTAL_P, &
                                        RHO(I,J,K)*SOOT_MASS_FRACTION/soot_density, X_H2O, X_CO2)
               else
                   call get_k_a_oneband(r_radlib, kabs, awts, IBND, TMP(I,J,K), TOTAL_P, &
                                        RHO(I,J,K)*SOOT_MASS_FRACTION/soot_density, X_H2O, X_CO2)
               endif
               KAPPA_GAS(I,J,K) = kabs
               BBF = awts
      ...
      DO IW = 1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
         ...
         ELSE
            ...
            IF (WSGG_MODEL) THEN
               ...
               X_CO2 = GET_VOLUME_FRACTION(CO2_INDEX,Z_ARRAY,R_MIXTURE)
               if (IBND==5) then         ! FDS has the clear gas last, radlib clear gas is first: FDS: 1,2,3,4,(5); radlib (0),1,2,3,4 (indicies match except clear)
                   call get_k_a_oneband(r_radlib, kabs, awts, 0,    WALL(IW)%ONE_D%TMP_F, 101325.0_EB, 0.0_EB, X_H2O, X_CO2)
               else
                   call get_k_a_oneband(r_radlib, kabs, awts, IBND, WALL(IW)%ONE_D%TMP_F, 101325.0_EB, 0.0_EB, X_H2O, X_CO2)
               endif
               BBF = awts
               !^^^^^^^^^^ dol

               !BBF = A_WSGG(WALL(IW)%ONE_D%TMP_F,MOL_RAT,IBND) !dol comment this! Temperature coefficient for the jth gray gas in the boundary
